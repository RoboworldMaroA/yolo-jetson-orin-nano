...
def save_image():
    ...
    try:
        ...
        if latest_frame is not None:
            ...
            r = requests.post(
                'https://172.17.0.2:5010/save_snapshot', timeout=1.0,verify=False,
            )
            return (
...
def start_recording():
    ...
    try:
        ...
        # )
        requests.post(
        'https://172.17.0.2:5010/set_recording_status', 
        json={'enabled': True, 'width': width, 'height': height, 'fps': fps}, 
...
def stop_recording():
    ...
    try:
        ...
        # )
        requests.post(
            'https://172.17.0.2:5010/set_recording_status',  # Użyj IP Dockera (widocznego w logach)
            json={'enabled': False},
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5055, threaded=True, ssl_context=('cert.pem', 'key.pem'))